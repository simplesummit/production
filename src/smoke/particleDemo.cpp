...
ParticleSystem *psystem_2 = 0;

SmokeRenderer *renderer = 0;
GLSLProgram *floorProg = 0;

int winWidth = 5, winHeight = 5;
int g_TotalErrors = 0;
...
vec3f cameraPos(-2, -2, -2);
vec3f cameraRot(30, -45, 0);
vec3f cameraPosLag(cameraPos);
vec3f cameraRotLag(cameraRot);
vec3f cursorPos(0, 1.5f, 0);
...
void initParticles(int numParticles, bool bUseVBO, bool bUseGL)
{
    ...
    psystem_2 = new ParticleSystem(numParticles, bUseVBO, bUseGL);
    psystem_2->reset(ParticleSystem::CONFIG_RANDOM);
...
void HUD() {
	// top left corner is origin
        glColor3f(0, 0.329f,0.141);
        glBegin(GL_QUADS);
                glVertex2f(.73*1024, 175.0);
                glVertex2f(.7*1024, 100.0f);
                glVertex2f(1024, 100.0f);
                glVertex2f(1024, 175.0f);
        //glBegin(GL_QUADS);
	glColor3f(0.0f, 0.47f, 0.2f); 
                glVertex2f(.735*1024, 170.0); 
                glVertex2f(.705*1024, 95.0f); 
                glVertex2f(1024, 95.0f);
                glVertex2f(1024, 170.0f);
...
void drawHUD() {
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(0, 1024, 720, 0.0, -1.0, 10.0);
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glLoadIdentity();
	...
	if (stat = 1) {
		...
		glEnable (GL_DEPTH_TEST);     //turn depth test back o
		glMatrixMode(GL_MODELVIEW);
		glPopMatrix();
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
...
void display()
{
    ...
    if (!paused)
    {
	    //probably where MPI code will begin: MPI_Init() as well as inside psystem stuff
        if (emitterOn)
        {
            runEmitter();
        }
...
*/
    ...
    //colorAttenuation[0] = std::abs(x/100);
    //colorAttenuation[1] = std::abs(y/100);
    //colorAttenuation[2] = std::abs(x/100);
...
int js[4];
int val;
void joystick_cb(unsigned int buttonMask, int x, int y, int _z)
{
    colorAttenuation[0];
    JoystickEvent event;
...
void initGL(int *argc, char **argv)
{
    ...
    char *imagePath = "../src/smoke/data/floortile.ppm";
    if (imagePath == NULL)
    {
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        glutJoystickFunc(joystick_cb, 10);
	glutTimerFunc(100, timer_cb, 0);
        //glutMouseFunc(mouse);
