...
int ox, oy, oz;
int buttonState = 0;
float camera_trans[] = {0, 0.42f, -2};
float camera_rot[] = {0, 0, 0};
float camera_trans_lag[] = {0, 0.42f, -2};
...
const char *sSDKsample = "CUDAParticles Simulation";
...
void initGL(int *argc, char **argv) {
  ...
  glutCreateWindow("CUDAParticles");
...
void computeFPS() {
  ...
  if (fpsCount == fpsLimit) {
    ...
    sprintf(fps, "CUDAParticles (%d particles): %3.1f fps", numParticles,
            ifps);
...
void motion(int x, int y, int z) {
  ...
  /*camera code
    if (buttonState == 3)
      {
        // left+middle = zoom
        camera_trans[2] += (dy1 / 100.0f) * 0.5f * fabs (camera_trans[2]);
      }
    else if (buttonState == 2)
      {
        // middle = translate
        camera_trans[0] += dx1 / 100.0f;
        camera_trans[1] -= dy1 / 100.0f;
      }
    else if (buttonState & 1)
      {
        // left = rotate
        camera_rot[0] += dy1 / 5.0f;
        camera_rot[1] += dx1 / 5.0f;
      }
  ...
  */
  ...
  if (buttonState == 1) {
    float v[3], r[3];
    v[0] = x * translateSpeed;
    v[1] = -y * translateSpeed;
    v[2] = z * translateSpeed;
    ...
    v[0] = 0.0f;
    v[1] = 0.0f;
    v[2] = y * translateSpeed;
    ixform(v, r, modelView);
    // std::cout << "ball x:" << p.x << "ball y: " << p.y;
    p.x += r[0];
    p.y += r[1];
    p.z += r[2];
  }
...
void joystick_cb(unsigned int buttonMask, int x, int y, int _z) {
  ...
  buttonState = 1;
  // std::cout << "x: " << x/10 << "z: " << z/32 << "\n";
  motion(x / 10, z / 200, y / 10);
}
